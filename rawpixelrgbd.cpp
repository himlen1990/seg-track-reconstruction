...
public:
  ...
  void process()
  {
    ...
    try{
      ...
      float ImgvarRGB=calImgVar(_imageRGB);
      float ImgvarD=calImgVar(_imageD);
      cout<<"imgvarRGB"<<ImgvarRGB<<endl;
      cout<<"imgvarD"<<ImgvarD<<endl;
#if 1
	...
	for(int j=0; j<_imgWidth-1; j++)
	{
	  ...
	  if(Disrgb_ij>DisDep_ij)
	    {   Gij=cv::exp(-(Disrgb_ij* Disrgb_ij)/(2*ImgvarRGB));
	      testcounter1++;
	    }
	  else
	    {
	    Gij=cv::exp(-(DisDep_ij* DisDep_ij)/(2*ImgvarD));
	      testcounter2++;
	    }
